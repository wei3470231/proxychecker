...
高性能代理 IP 采集、检测、分发系统。Go 语言构建，SOCKS5/HTTP/HTTPS/SOCKS4 全协议代理池管理，多设备协同。

支持闲置安卓手机部署该项目，只需要安装termux，将编译二进制文件和配置文件导入termux，即可多设备采集、校验、筛选、匿名代理。完全解耦，可以把手机和电脑混合在一起采集、校验IP，并统一分发。
